...
public class TheaterSeatSeller {
	...
	public TheaterSeatSeller() {
		for (int i = 0; i < NUM_ROWS; i++){
			...
			pricesAvailable[((seats[i][j]/10)-1)]++;
	...
	public void printSeats() {
		...
		System.out.println("Tickets available by price: ");//Constructs a table of how many seats of each price is avaliable
	...
	public boolean getByPrice(int price) {
		...
		for (int i = 0; i < NUM_ROWS; i++){ // Process the ith row
					
											
		for (int j = 0; j < NUM_COLS; j++){ // Process the jth column
			if (price== 10 && seats[i][j]==10){			// Tests if the price entered is 10 and if the seat location is 10
				seats[i][j] =0 ;						//If it is successful, it will replace the location with 0
				retVal=true;							//	Returns True if seat found
				pricesAvailable[0]--;					//	Updates the pricesAvaliable array by decrementing by 1
				break;									// breaks out of the loop
			}
														
			else if (price== 20 && seats[i][j]==20){    // Tests if the price is 20 and if the seat location is 20
				seats[i][j] =0 ;						// If it is successful, it will replace the location with 0
				retVal=true;							//	Returns True if seat found
				pricesAvailable[1]--;					//Updates the pricesAvaliable array by decrementing by 1
				break;									//breaks out of the loop
			
			}
			else if (price== 30 && seats[i][j]==30){	//Tests if the price is 30 and if the seat location is 30
				seats[i][j] =0 ;						//	If it is successful, it will replace the location with 0
				retVal=true;							//	Returns True if seat found
				pricesAvailable[2]--;					//Updates the pricesAvaliable array by decrementing by 1
				break;									//breaks out of the loop
			
			}
			else if (price== 40 && seats[i][j]==40){	//Tests if the price is 40 and if the seat location is 40
				seats[i][j] =0 ;						//If it is successful, it will replace the location with 0
				retVal=true;							//Returns True if seat found
				pricesAvailable[3]--;					//Updates the pricesAvaliable array by decrementing by 1
				break;									//breaks out of the loop
			
			} 
			else if (price== 50 && seats[i][j]==50){	//Tests if the price is 50 and if the seat location is 50
				seats[i][j] =0 ;						//If it is successful, it will replace the location with 0
				retVal=true;							//Returns True if seat found
				pricesAvailable[4]--;					//Updates the pricesAvaliable array by decrementing by 1
				break;									//breaks out of the loop
			...
			if (retVal==true)							// Tests to see if the previous statements returned a retVal of True
				break;									// If it was true, it breaks out of the loop
	...
	public boolean getByLoc(int row, int col) {
		...
		//************YOUR SOLUTION GOES HERE************//
		if (seats[row][col]==10){						//Tests if the value at the row and column that the user entered equals 10,20,30,40,50
				seats[row][col] =0 ;					//If the location is found with the respective price, changes the value to 0 to represent it is sold
				retVal=true;							// Returns True if the seat is found
				pricesAvailable[0]--;					//Updates the prices availiable array.
														//
			}											 
			else if (seats[row][col]==20){				//Same as above
				seats[row][col] =0 ;					
				retVal=true;							
				pricesAvailable[1]--;					
			...
			else if (seats[row][col]==30){				//Same as above
				seats[row][col] =0 ;
				retVal=true;
				pricesAvailable[2]--;
			...
			else if (seats[row][col]==40){				//Same as above
				seats[row][col] =0 ;
				retVal=true;
				pricesAvailable[3]--;
			...
			else if (seats[row][col]==50){				//Same as above
				seats[row][col] =0 ;
				retVal=true;
				pricesAvailable[4]--;
			...
			return retVal; //return value				//Returns retVal value
